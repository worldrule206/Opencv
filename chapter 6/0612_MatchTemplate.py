...
tmp_d   = cv2.imread('../data/D.bmp', cv2.IMREAD_GRAYSCALE)
...
#3
R4 = cv2.matchTemplate(src, tmp_d, cv2.TM_CCOEFF_NORMED)
_, maxVal, _, maxLoc = cv2.minMaxLoc(R4)
print('TM_CCOEFF_NORMED:', maxVal, maxLoc)
w, h = tmp_d.shape[:2]
cv2.rectangle(dst, maxLoc, (maxLoc[0]+h, maxLoc[1]+w), (0, 255, 255), 2)
